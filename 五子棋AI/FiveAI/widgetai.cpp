...
void Widget::getAvaliablePos() {
    ...
    for (int i = 0; i < 15; ++i) {
        for (int j = 0; j < 15; ++j) {
            ...
            for (int k = 0; k < winsWayCount; ++k) {
                    if (playerWinsWayStatisticArr[k] >= 3 || computerWinsWayStatisticArr[k] >= 3) {
                        avaliableStep.push_back(QPoint(i,j));
                    }
            }
        }
    }
...
void Widget::rollback(int r, int c) {
    playMap[r][c]=0;
...
int Widget::deep_max(int deep, int parentBeta, bool con) {
    ...
    for (auto it = avaliableStep.begin(); it != avaliableStep.end(); ++it) {
        set_a_chess(it->x(), it->y(), 'p');
        hasWin = false;

//        for (int i = 0; i < winsWayCount; ++i) {
//            // 靠近胜利+1
//            if (winsArr[it->x()][it->y()][i]) {
//                playerWinsWayStatisticArr[i]++;
//                // i方法下人类无法赢
//                buffAiWins[i] = computerWinsWayStatisticArr[i];
...
//        for (auto it = caseNum.begin(); it != caseNum.end(); ++it) {
//            computerWinsWayStatisticArr[*it]= buffAiWins[*it];
//            playerWinsWayStatisticArr[*it]--;
//        }
...
int Widget::deep_min(int deep, int parentAlpha, bool con) {
    ...
    bool hasWin = false;
    int buffPlayerWins[572];
    QList<int> caseNum;
    for (auto it = avaliableStep.begin(); it != avaliableStep.end(); ++it) {
        set_a_chess(it->x(), it->y(), 'c');
        hasWin = false;
...
//                computerWinsWayStatisticArr[i]++;
//                // i方法下人类无法赢
//                buffPlayerWins[i] = playerWinsWayStatisticArr[i];
//                playerWinsWayStatisticArr[i] = 10;
//                caseNum.push_back(i);
//                if (computerWinsWayStatisticArr[i] == 5) {
//                    hasWin = true;
...
//        for (auto it = caseNum.begin(); it != caseNum.end(); ++it) {
//            playerWinsWayStatisticArr[*it]= buffPlayerWins[*it];
//            computerWinsWayStatisticArr[*it]--;
//        }
