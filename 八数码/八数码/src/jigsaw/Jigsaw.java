...
public class Jigsaw {
	 ...
	 */
	private void sortedInsertOpenList(JigsawNode jNode, int type) {
		this.estimateValue(jNode, type);
	 ...
	 * @throws IOException
	 */
	public boolean ASearch(int type) throws IOException{
		// 将搜索过程写入ASearchDialog.txt
		String filePath = "ASearchDialog"+ String.valueOf(type) +".txt";
		PrintWriter pw = new PrintWriter(new FileWriter(filePath));
		this.openList.clear();
		this.closeList.clear();
		// 访问节点数大于30000个则认为搜索失败
		...
		// (1)将起始节点放入openList中
		this.sortedInsertOpenList(this.beginJNode, type);
		...
		while (this.openList.isEmpty() != true && searchedNodesNum <= maxNodesNum) {
			...
			followJNodes = this.findFollowJNodes(this.currentJNode);
			while (!followJNodes.isEmpty()) {
				this.sortedInsertOpenList(followJNodes.elementAt(0), type);
				followJNodes.removeElementAt(0);
			}
		}
	 ...
	 *	 
	 */
	private void estimateValue(JigsawNode jNode, int type) {
		int sumOfWrongPosDis_val = sumOfWrongPosDis(jNode);
		int numOfWrongPos_val = numOfWrongPos(jNode);
		int numOfWrongPosBehind_val = numOfWrongPosBehind(jNode);
		// jNode.setEstimatedValue( 2*sumOfWrongPosDis_val+3*numOfWrongPos_val+numOfWrongPosBehind_val);
		if (type == 0) {
			// 曼哈顿距离
			jNode.setEstimatedValue( sumOfWrongPosDis_val);
		} else {
			// 放错的数量
			jNode.setEstimatedValue( numOfWrongPos_val);
		}
	 ...
	 * 所有 放错位的数码 个数
	 */
	private int numOfWrongPos(JigsawNode jNode) {
		int count = 0;
		for (int i = 1 ; i <= jNode.getDimension()*jNode.getDimension(); ++i) {
	...
	/**
	 * 所有 放错位的数码与其正确位置距离 之和
	 */
	private int sumOfWrongPosDis(JigsawNode jNode) {
		int dis = 0;
		int dim = jNode.getDimension();
		for (int i = 1; i <= dim * dim; i++) {
			if (jNode.getNodesState()[i] != 0 && jNode.getNodesState()[i] != i) {
				int aimR = (jNode.getNodesState()[i] - 1) / dim;
				int aimC = (jNode.getNodesState()[i] - 1) % dim;

				int curR = (i - 1) / dim;
				int curC = (i - 1) % dim;
				
				dis += Math.abs(aimC - curC) + Math.abs(aimR - curR); 
			}
		}
		return dis;
	}
	 ...
	 * 后续节点不正确的数码个数
	 */
	private int numOfWrongPosBehind(JigsawNode jNode) {
		int count = 0;
		int dim = jNode.getDimension();
